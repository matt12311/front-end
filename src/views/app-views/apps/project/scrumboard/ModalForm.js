...
import { NewMembersData } from 'views/app-views/dashboards/default/DefaultDashboardData';
...
const UpdateCardForm = ({ onSubmit, cardData, listId, board_id, members,user }) => {
				...
				>
					{
						members.map((elm) => (
							<Option key={elm._id} value={elm._id}>
								<AssigneeAvatar name={elm.name} id={elm._id}></AssigneeAvatar>
							</Option>
						))
					}
				</Select>
						...
						<div className="w-100">
							...
							<div className="mb-3 d-flex">
								<Avatar  style={{
									backgroundColor: "#7265e6",
									verticalAlign: 'middle',
									}}   size="large"  >{user.firstName}</Avatar>
								<div className="ml-2 bg-gray-lightest p-3 rounded w-100">
									<Input
										ref={commentInput}
										...
										suffix={
											<div
												onClick={() => submitComment(user.firstName,user.photoPath)}
...
const ModalForm = ({ visible, modalMode, cardData, listId, board_id, onClose, onModalSubmit, members, addNotification,user }) => {
	...
	const submit = (values, mode) => {
		...
		if (values.members && values.members.length > 0) {//noti burda
			let body = {
				title: values.name + "  Task Updated.",
				description: "",
