...
import { UserOutlined, LockOutlined, CreditCardOutlined, ShopOutlined, TeamOutlined} from '@ant-design/icons';
...
const SettingOption = ({ match, location }) => {
	return (
		...
		>
			...
			<Menu.Item key={`${match.url}/shopify`}>
			<ShopOutlined />
			<span>Shopify</span>
				<Link to={`shopify`} />
			</Menu.Item>
			<Menu.Item key={`${match.url}/team`}>
			<TeamOutlined />
			<span>Team</span>
				<Link to={`team`} />
			</Menu.Item>
...
const SettingContent = ({ match }) => {
	return (
		<Switch>
			...
			<Route path={`${match.url}/shopify`} component={Shopify} />
